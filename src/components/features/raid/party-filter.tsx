...
interface PartyFilterProps {
  ...
  // 풀 필터 활성 시 include/exclude/hardExclude 비활성화
  poolActive?: boolean;
}
...
function PartyFilterComponent({
  ...
  return (
    <>
      ...
      {/* 포함/제외/hardExclude: 풀 필터 활성 시 비활성화 */}
      <fieldset
        disabled={poolActive}
        className={poolActive ? "opacity-50 pointer-events-none" : ""}
      >
        {poolActive && (
          <p className="text-xs text-muted-foreground mb-3">
            풀 필터 사용 중 — 캐릭터 포함/제외는 풀 설정으로 대체됩니다.
          </p>
        )}
